...
checkStreetName xs = check (length xs <= 20) "Invalid street name" xs

checkStreetNumber :: String -> Validation String
checkStreetNumber xs = check (all isDigit xs) "Invalid street number" xs

checkPostcode :: String -> Validation String
checkPostcode xs =
  check (length xs == 5 && all isDigit xs) "Invalid postcode" xs
...
boolOrInt x = bval <|> ival
  where
    bval = case readMaybe x :: Maybe Bool of
      Just b -> check True "" (Left b)
      Nothing -> invalid "Not a Bool"
    ival = case readMaybe x :: Maybe Int of
...
normalizePhone num = checkLength num' *> traverse checkDigit num'
  where
    checkLength x = check (length x <= 10) "Too long" x
    checkDigit ch = check (isDigit ch) ("Invalid character: " ++ [ch]) ch
    num' = filter (/= ' ') num
...
parseExpression input = case words input of
  ...
  where
    ...
    parseVar arg =
      check
        (length arg == 1 && isAlpha (head arg))
        ("Invalid variable: " ++ arg)
        (Variable $ head arg)
    parseNum arg = case (readMaybe arg :: Maybe Int) of
