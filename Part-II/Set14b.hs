...
-- Database
import Database.SQLite.Simple (Connection, Query (..), ToRow (..), execute, execute_, open, query, query_)
import Mooc.Todo
...
balance db account = do
  res <- query db balanceQuery [account] :: IO [[Maybe Int]]
  pure . sum . maybeToList . head . head $ res
...
perform db (Just (Deposit account amount)) =
  deposit db account amount *> pure (T.pack "OK")
